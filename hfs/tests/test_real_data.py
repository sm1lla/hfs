import numpy as np
import pytest

from hfs.preprocessing import HierarchicalPreprocessor


from ..filter import Filter
from ..hnb import HNB
from .fixtures.fixtures import *



import pytest

@pytest.fixture
def data1():
    graph = nx.read_gml("../hfs/hfs/data/go_digraph2.gml")
    subgraph = graph.subgraph([
        "GO:2001301",
        "GO:2001302",
        "GO:2001303",
        "GO:2001304",
        "GO:2001305",
        "GO:2001306",
        "GO:2001307",
        "GO:2001308",
        "GO:2001309",
        "GO:2001310",
        "GO:2001311",
        "GO:2001312",
        "GO:2001313",
        "GO:2001314",
        "GO:2001315",
        "GO:2001316",
        "GO:2001092",
        "GO:2001094",
        "GO:2001106",
        "GO:2001107",
    ] )
    df = pd.read_csv("../hfs/hfs/data/train_test.csv")
    train = df[df["split"]=="train"]
    test = df[df["split"]=="test"]
    g2g = pd.read_csv(r"../hfs/hfs/data/gene2gomod3.txt",sep=",")
    go_terms = g2g["GO_ID"].unique()
    y_train = train["longevity influence"].to_numpy()
    y_test = test["longevity influence"].to_numpy()
    train = train[go_terms].to_numpy()
    test = test[go_terms].to_numpy()
    hierarchy = nx.to_numpy_array(subgraph)
    #hierarchy = np.load("../hfs/hfs/data/go_digraph_np.npy", allow_pickle=True)
    return (hierarchy, train, y_train, test, y_test)

@pytest.fixture
def data2():
    graph = nx.read_gml("../hfs/hfs/data/go_digraph2.gml")
    subgraph = graph.subgraph([
        "GO:2001301",
        "GO:2001302",
        "GO:2001303",
        "GO:2001304",
        "GO:2001305",
        "GO:2001306",
        "GO:2001307",
        "GO:2001308",
        "GO:2001309",
        "GO:2001310",
        "GO:2001311",
        "GO:2001312",
        "GO:2001313",
        "GO:2001314",
        "GO:2001315",
        "GO:2001316",
        "GO:2001092",
        "GO:2001094",
        "GO:2001106",
        "GO:2001107",
    ] )
    df = pd.read_csv("../hfs/hfs/data/train_test.csv")
    train = df[df["split"]=="train"]
    test = df[df["split"]=="test"]
    g2g = pd.read_csv(r"../hfs/hfs/data/gene2gomod3.txt",sep=",")
    go_terms = g2g["GO_ID"].unique()
    y_train = train["longevity influence"].to_numpy()
    y_test = test["longevity influence"].to_numpy()
    # give position of feature in graph
    columns = [list(subgraph.nodes()).index(node) for node in train.columns if node in subgraph.nodes]
    train = train[go_terms].to_numpy()
    test = test[go_terms].to_numpy()
    hierarchy = nx.to_numpy_array(subgraph)
    return (hierarchy, train, y_train, test, y_test, columns)


def test_preprocessing(data2):
    hierarchy, train, y_train, test, y_test, columns = data2
    preprocessor = HierarchicalPreprocessor(hierarchy=hierarchy)
    #match features to nodes:
    # each nodes 
    preprocessor.fit(train, columns=columns)

# Test feature selection of HNB
def test_HNB(data1):
    hierarchy, train, y_train, test, y_test = data1
    filter = HNB(hierarchy=hierarchy, k=2)
    filter.fit_selector(X_train=train, y_train=y_train, X_test=test)
    pred = filter.select_and_predict(predict=True, saveFeatures=True)
